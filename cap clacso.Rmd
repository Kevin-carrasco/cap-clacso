...
proporcion <- prop.table(table(data$desigualdad)) * 100
proporcion <- as.data.frame(proporcion)

desigualdad_est <- proporcion %>% 
...
ggsave(desigualdad_est, file="output/graphs/desigualdad_est.png", width = 7,height = 7)
```
...
ggsave(desigualdad_educ, file="output/graphs/desigualdad_educ.png", width = 9.5,height = 7)
```
...
des_mersoc <- data %>% filter(desigualdad_rec!=is.na(desigualdad_rec) & merito_soc_rec!=is.na(merito_soc_rec)) %>% 
  ...
  labs(y = "Preocupación por la desigualdad",
